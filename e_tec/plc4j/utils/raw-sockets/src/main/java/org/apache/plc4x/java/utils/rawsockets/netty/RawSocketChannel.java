...
public class RawSocketChannel extends OioByteStreamChannel {
        ...
        if((receiveHandle != null) && (receiveHandle.isOpen())){
            ...
            if(filter.length() > 0) {
                try {
                    ...
                    throw new IllegalArgumentException("Error updating filter expression");
                }
            }
        }
    }
    ...
    protected void doConnect(SocketAddress remoteAddress, SocketAddress localAddress) throws Exception {
        ...
        if (nif == null) {
            logger.info("Couldn't find network device for %s");
            pipeline().fireExceptionCaught(
        ...
        if(config.isResolveMacAddress() &&(this.remoteMacAddress == null) && (this.remoteAddress instanceof InetSocketAddress)) {
         
                this.remoteMacAddress = ArpUtils.resolveMacAddress(nif, 
                    (InetSocketAddress) this.remoteAddress,
                    (InetSocketAddress) this.localAddress, 
                    localMacAddress).orElse(null);
        ...
        // Make configurable, if we should accept from any source or not.
        MacAddress tempRemoteMacAddress = remoteMacAddress != null ? remoteMacAddress : MacAddress.getByAddress(new byte[]{0,0,0,0,0,0});
        String filter = config.getMacBasedFilterString(localMacAddress, tempRemoteMacAddress);
        if(filter.length() > 0) {
            receiveHandle.setFilter(filter, BpfProgram.BpfCompileMode.OPTIMIZE);
        ...
        loopThread = new Thread(() -> {
            try {
                ...
                // this should close everything automatically
    ...
    private boolean pezzoCod1(InetAddress address,PcapNetworkInterface dev){
        ...
        if(subnetInfo.isInRange(address.getHostAddress())) {
            ...
            this.localMacAddress = dev.getLinkLayerAddresses().stream()
                .filter(MacAddress.class::isInstance).map(MacAddress.class::cast)
    ...
    private static class PcapOutputStream extends OutputStream {
        ...
        public void write(int b) throws IOException {
            ...
            throw new IllegalArgumentException("write(byte) should never be called in a RawSocketChannel");
        }
    ...
    private static class PcapInputStream extends InputStream {
        ...
        @Override
        public int read(byte[] b, int i1, int i2) throws IOException {
            return buf.readByte();
        }
