...
public class DefaultPlcSubscriptionRequest implements PlcSubscriptionRequest, Serializable {
    ...
    private final Map<String, PlcSubscriptionTag> tags;

    private final Map<String, List<Consumer<PlcSubscriptionEvent>>> preRegisteredConsumers;

    @JsonCreator(mode = JsonCreator.Mode.PROPERTIES)
    public DefaultPlcSubscriptionRequest(@JsonProperty("subscriber") PlcSubscriber subscriber,
                                         @JsonProperty("tags") Map<String, PlcSubscriptionTag> tags,
                                         @JsonProperty("preRegisteredConsumers") Map<String, List<Consumer<PlcSubscriptionEvent>>> preRegisteredConsumers) {
    ...
    public void serialize(WriteBuffer writeBuffer) throws SerializationException {
        ...
        for (Map.Entry<String, PlcSubscriptionTag> tagEntry : tags.entrySet()) {
            ...
            if (!(tag instanceof Serializable)) {
                throw new IllegalArgumentException("Error serializing. Tag doesn't implement XmlSerializable");
            }
        ...
        public PlcSubscriptionRequest build() {
            ...
            preRegisteredConsumers.forEach((tagName, ignored) -> {
                if (!tags.containsKey(tagName)) {
                    throw new IllegalArgumentException("tagName " + tagName + "for preRegisteredConsumer not found");
                }
            });
