...
public class AsciiBox {
    ...
    private final Pattern ansiPattern = Pattern.compile("[\u001b\u009b][\\[()#;?]*(?:\\d{1,4}(?:;\\d{0,4})*)?[0-9A-ORZcf-nqry=><]");

    private final AsciiBoxWriter asciiBoxWriter;

    private final String data;

    //should be final but for the moment we mutate it in change box name... Maybe we find another solution
    ...
    public int width() {
        ...
        for (String line : lines()) {
            int currentLength = ansiPattern.matcher(line).replaceAll("").length();
