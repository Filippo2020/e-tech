...
public abstract class GeneratedDriverBase<B extends Message> implements PlcDriver {
    ...
    public ProtocolStackConfigurer<B> getStackConfigurer() {
        return new ProtocolStackConfigurer<B>() {
            @Override
            public Plc4xProtocolBase<B> configurePipeline(Configuration configuration, ChannelPipeline pipeline,
                    PlcAuthentication authentication, boolean passive, List<EventListener> listeners) {
                throw new UnsupportedOperationException("Unimplemented method 'configurePipeline'");
            }
