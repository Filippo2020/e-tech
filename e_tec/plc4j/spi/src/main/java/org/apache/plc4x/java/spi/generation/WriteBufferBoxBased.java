...
public class WriteBufferBoxBased implements WriteBuffer, BufferCommons {
    ...
    private static final int DESIREWIDTH = 120;
    private final boolean mergeSingleBoxes;
    private final boolean omitEmptyBoxes;
    private int currentWidth =  DESIREWIDTH - 2;
    private static final String S1="x %d%s";
    private int pos = 1;

    private String repl = "%s %d %s %d %d %s";
    ...
    public void writeUnsignedByte(String logicalName, int bitLength, byte value, WithWriterArgs... writerArgs) throws SerializationException {
        String additionalStringRepresentation = extractAdditionalStringRepresentation(writerArgs).map(s -> " " + s).orElse("");
        boxes.offerLast(Either.left(asciiBoxWriter.boxString(logicalName, String.format(repl, "0x%0", Math.max(bitLength / 4, 1), S1, value, value, additionalStringRepresentation), 0)));
        move(bitLength);
    }
    ...
    public void writeFloat(String logicalName, int bitLength, float value, WithWriterArgs... writerArgs) throws SerializationException {
        String additionalStringRepresentation = extractAdditionalStringRepresentation(writerArgs).map(s -> " " + s).orElse("");
        boxes.offerLast(Either.left(asciiBoxWriter.boxString(logicalName, String.format(repl, "0x%0", Math.max(bitLength / 4, 1), "x %f%s", value, value, additionalStringRepresentation), 0)));
        move(bitLength);
    }
    ...
    public void writeVirtual(String logicalName, Object value, WithWriterArgs... writerArgs) throws SerializationException {
        ...
        if (value instanceof String) {
            ...
            virtualBox = asciiBoxWriterLight.boxString(logicalName, String.format("0x%x %s%s", number, number, additionalStringRepresentation), 0);
        } else if (value instanceof Boolean) {
            Boolean v=(Boolean) value;
            virtualBox = asciiBoxWriterLight.boxString(logicalName, String.format("b%b %s%s", v, value, additionalStringRepresentation), 0);
    ...
    public void cod1(Deque<AsciiBox> finalBoxes, String logicalName){
