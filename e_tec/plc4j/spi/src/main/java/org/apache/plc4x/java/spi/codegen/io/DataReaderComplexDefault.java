...
public class DataReaderComplexDefault<T> implements DataReaderComplex<T> {
    ...
    //maybe replace with resetable or something so its clear that that's the only purpose
    ...
    public T read(String logicalName, ComplexTypeSupplier<T> complexSupplier, WithReaderArgs... readerArgs) throws ParseException {
        //it might be even better if we default to value like in other places... on the other hand a complex type has always a proper logical name so this might be fine like that
