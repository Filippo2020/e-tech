...
public class S7PlcToGoogleIoTCoreSample {
        ...
        MqttCallback mCallback = new MqttCallback() {
            ...
            public void messageArrived(String topic, MqttMessage message) {
                ...
                if(logger.isDebugEnabled()){
                    String a1 =String.format("Payload : %s",payload);
                    logger.debug(a1);// : Insert your parsing / handling of the configuration message here.
                }         
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        MqttClient client = new MqttClient(mqttServerAddress, mqttClientId, new MemoryPersistence())) {
            ...
            while (!client.isConnected() && totalRetryTimeMs < maxConnectRetryTimeElapsedMillis) {
                try {
                    ...
                    if(logger.isDebugEnabled())
                    {
                    String frase="An error occurred: " + e.getMessage();
                    logger.debug(frase);
                    }
                    if (reason == MqttException.REASON_CODE_CONNECTION_LOST
                        || reason == MqttException.REASON_CODE_SERVER_CONNECT_ERROR) {
                        String frase="Retrying in " + retryIntervalMs / 1000.0 + " seconds.";
                        logger.debug(frase);
            ...
            try (PlcConnection plcConnection = new PlcDriverManager().getConnection("s7://10.10.64.20/1/1")) {
                ...
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    if (secsSinceRefresh > (options.getTokenExpMins() * 60)) {
                        String frase= String.format("\tRefreshing token after: %d seconds%n", secsSinceRefresh);
                        logger.debug(frase);
                        iat = new DateTime();
                        setConnectPassword(options, connectOptions);
                        client.disconnect();
        ...
        logger.debug("Sent all messages. Goodbye!");
