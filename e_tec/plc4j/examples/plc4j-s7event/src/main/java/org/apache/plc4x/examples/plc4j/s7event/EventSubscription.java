    ...
    public static void main(String[] args) throws Exception {
        ...
        try (PlcConnection connection = new PlcDriverManager().getConnection("s7://192.168.1.51?remote-rack=0&remote-slot=3&controller-type=S7_400")) {
            ...
            subresponse
                ...
                .register(msg -> {
                    logger.log(new LogRecord(Level.ALL,"******** S7SysEvent ********"));
            ...
            subresponse
                ...
                .register(msg -> {
                    logger.log(new LogRecord(Level.ALL,"******** S7UserEvent *******"));
                    Map<String, Object> map = ((S7UserEvent) msg).getMap();
                    map.forEach((x, y) -> 
                        logger.log(new LogRecord(Level.ALL, x + " : " + y))
                    );
                    logger.log(new LogRecord(Level.ALL, stelle));
                });

            subresponse
                .getSubscriptionHandle("myALM")
                .register(msg -> {
                 logger.log(new LogRecord(Level.ALL,"******** S7AlmEvent *********"));
           ...
           logger.log(new LogRecord(Level.ALL,"Waiting for events"));

            Thread.sleep(120000);

            logger.log(new LogRecord(Level.ALL, "Bye..."));
