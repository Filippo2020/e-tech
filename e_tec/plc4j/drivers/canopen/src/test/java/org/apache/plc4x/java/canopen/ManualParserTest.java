...
public class ManualParserTest {
    ...
    @Test
    void readBufferTest() throws Exception {
        ...
        assertEquals(0x5A1, value);
    }

    @Test
     void standardFrameParser() throws Exception {
        SocketCanFrameStub frame = parse(STANDARD);
        //System.out.println(frame);

        assertEquals(0x5A1, frame.id);
        assertEquals(false, frame.extended);
        assertEquals(false, frame.remote);
        assertEquals(false, frame.error);
        assertEquals(8, frame.data.length);
    }

    @Test
    void extendedFrameParser() throws Exception {
        SocketCanFrameStub frame = parse(EXTENDED);
        //System.out.println(frame);

        assertEquals(0x1e6ec676, frame.id);
        assertEquals(true, frame.extended);
        assertEquals(false, frame.remote);
        assertEquals(false, frame.error);
        assertEquals(5, frame.data.length);
