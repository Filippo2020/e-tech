...
public class ProfinetPlcDiscoverer implements PlcDiscoverer {
    ...
    private static final String S1="unknown";
    ...
    public CompletableFuture<PlcDiscoveryResponse> discoverWithHandler(PlcDiscoveryRequest discoveryRequest, PlcDiscoveryItemHandler handler) {
        ...
        cod2(timer, future, discoveryRequest, values, openHandles);

        return future;
    }

    public StringBuilder cod1(Map<String, PnDcp_Block> blocks, StringBuilder role){
        if (blocks.containsKey(DEVICE_ROLE)) {
            ...
            if (block.getPnioMultidevive()) {
                role.append(",MULTIDEVICE");
            }
            if (block.getPnioController()) {
                role.append(",CONTROLLER");
            }
            ...
            if (role.length() > 0) {
                ...
                role = new StringBuilder(S1);
            }
        }
        return role;
    }

    public void cod2(Timer timer, CompletableFuture<PlcDiscoveryResponse> future, PlcDiscoveryRequest discoveryRequest, List<PlcDiscoveryItem> values, Set<PcapHandle> openHandles){
    ...
    public PlcDiscoveryItem cod3(Map<String, String> options, String remoteIpAddress, String remoteSubnetMask, 
                    org.pcap4j.util.MacAddress srcAddr, String deviceTypeName, String deviceName, String vendorId, 
                    org.pcap4j.util.MacAddress dstAddr, String deviceId, StringBuilder role, List<PlcDiscoveryItem> values){
        options.put("remoteIpAddress", remoteIpAddress);
                                                ...
                                                options.put("role", role.toString());
    ...
    public boolean cod4(EthernetPacket ethernetPacket, boolean isPnPacket){
    ...
    public void cod5(ReadBuffer reader, EthernetPacket ethernetPacket, PlcDiscoveryItemHandler handler, List<PlcDiscoveryItem> values) throws ParseException{
        Ethernet_Frame ethernetFrame = Ethernet_Frame.staticParse(reader);
                                            ...
                                            if (pdu instanceof PnDcp_Pdu_IdentifyRes) {
                                                ...
                                                role = cod1(blocks, role);

                                                String remoteIpAddress = S1;
                                                String remoteSubnetMask = S1;
                                                ...
                                                PlcDiscoveryItem value;
                                                value = cod3(options, remoteIpAddress, remoteSubnetMask, srcAddr, deviceTypeName, deviceName, vendorId, dstAddr, deviceId, role, values);
    ...
    public void cod6(Packet packet, PlcDiscoveryItemHandler handler, List<PlcDiscoveryItem> values) {
