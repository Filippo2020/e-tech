...
public class ProfinetProtocolLogic extends Plc4xProtocolBase<Ethernet_Frame> {
    ...
    private static final String S1="000000000000";
    ...
    public void onConnect(ConversationContext<Ethernet_Frame> context) {
        ...
        try(DatagramSocket udpSocket1 = new DatagramSocket()) {
                //comment
        } catch (SocketException e) {
            logger.warn(String.format("Unable to create udp socket %s",e.getMessage()));
            context.getChannel();
            return;
        }
        ...
        //Possibly we can remove the ARP lookup and simply use the mac address in the connection-response.
        ...
        try {
            ...
            final DceRpc_Packet dceRpcPacket = DceRpc_Packet.staticParse(readBuffer);
            if ((dceRpcPacket.getOperation() == DceRpc_Operation.CONNECT) && (dceRpcPacket.getPacketType() == DceRpc_PacketType.RESPONSE)) {
                if (dceRpcPacket.getPayload().getPacketType() == DceRpc_PacketType.RESPONSE) {
                    cod1(channel, dceRpcPacket);
                } else {
                    throw new PlcException("Unexpected response");
                }
            } else if (dceRpcPacket.getPacketType() == DceRpc_PacketType.REJECT) {
                throw new PlcException("Device rejected connection request");
            } else {
                throw new PlcException("Unexpected response");
            }
        } catch (SerializationException | PlcException | ParseException e) {
            logger.error("Error", e);
        }

    }

    public void cod1(Channel channel, DceRpc_Packet dceRpcPacket) throws PlcException{
        // Get the remote MAC address and store it in the context.
        final PnIoCm_Packet_Res connectResponse = (PnIoCm_Packet_Res) dceRpcPacket.getPayload();
        if ((!connectResponse.getBlocks().isEmpty()) && (connectResponse.getBlocks().get(0) instanceof PnIoCm_Block_ArRes)) {
            final PnIoCm_Block_ArRes pnIoCmBlockArRes = (PnIoCm_Block_ArRes) connectResponse.getBlocks().get(0);
            profinetDriverContext.setRemoteMacAddress(pnIoCmBlockArRes.getCmResponderMacAddr());
    ...
    @Override
    public void decode(ConversationContext<Ethernet_Frame> context, Ethernet_Frame msg) throws IllegalArgumentException {
        try{
                ...
                new PnIoCm_Packet_Req(404, 404, 404, 0, 404,
                    Arrays.asList(
                        ...
                        new PnIoCm_Block_IoCrReq((short) 1, (short) 0, PnIoCm_IoCrType.INPUT_CR,
                            ...
                            3, 3, 0xC000,
                            new org.apache.plc4x.java.profinet.readwrite.MacAddress(Hex.decodeHex(S1)),
                        ...
                        new PnIoCm_Block_IoCrReq((short) 1, (short) 0, PnIoCm_IoCrType.OUTPUT_CR,
                            ...
                            3, 3, 0xC000,
                            new MacAddress(Hex.decodeHex(S1)),
                            Collections.singletonList(
                                new PnIoCm_IoCrBlockReqApi(
                                    Collections.singletonList(
