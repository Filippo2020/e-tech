...
public class EipProtocolLogic extends Plc4xProtocolBase<EipPacket> implements HasConfiguration<EIPConfiguration> {
    ...
    public void onConnect(ConversationContext<EipPacket> context) {
        ...
        context.sendRequest(connectionRequest)
            ...
            .check(EipConnectionRequest.class::isInstance)
            .handle(p -> {
                if (p.getStatus() == 0L) {
    ...
    private CompletableFuture<PlcReadResponse> toPlcReadResponse(PlcReadRequest readRequest, CompletableFuture<CipService> response) {
        return response
            .thenApply(p -> (PlcReadResponse) decodeReadResponse(p, readRequest));
    }
    ...
    private CompletableFuture<CipService> readInternal(List<CipReadRequest> request) {
        ...
        if (request.size() > 1) {
            ...
            transaction.submit(() -> context.sendRequest(pkt)
                ...
                .check(CipRRData.class::isInstance)
                .check(p -> p.getSessionHandle() == sessionHandle)
                //.check(p -> p.getSenderContext() == senderContext)
                .unwrap(CipRRData.class::cast)
                .unwrap(p -> p.getExchange().getService()).check(MultipleServiceResponse.class::isInstance)
                .unwrap(MultipleServiceResponse.class::cast)
            ...
            transaction.submit(() -> context.sendRequest(pkt)
                ...
                //.check(p -> p.getSenderContext() == senderContext)
                .unwrap(CipRRData.class::cast)
                .unwrap(p -> p.getExchange().getService()).check(CipReadResponse.class::isInstance)
                .unwrap(CipReadResponse.class::cast)
                .handle(p -> {
                    future.complete(p);
                    // Finish the request-transaction.
                    transaction.endRequest();
    ...
    private PlcResponse decodeReadResponse(CipService p, PlcReadRequest readRequest) {
        ...
        //Multiple response
            ...
            pezzoCod3(p,readRequest, values);
        }
        return new DefaultPlcReadResponse(readRequest, values);
    }
    private void pezzoCod3(CipService p, PlcReadRequest readRequest, Map<String, ResponseItem<PlcValue>> values){
        ...
        for (int i = 0; i < nb && it.hasNext(); i++) {
            pezzoCod4(services,i, it, readRequest, values);
        }
    }
   
    private Map<String, ResponseItem<PlcValue>> pezzoCod4(Services services, int i, Iterator<String> it, PlcReadRequest readRequest, Map<String, ResponseItem<PlcValue>> values){
    ...
    private void pezzoCod(ByteBuf data, CIPDataTypeCode type, List<PlcValue> list, final int STRING_LEN_OFFSET, final  int  STRING_DATA_OFFSET, int index){
        switch (type) {
            case DINT:
            ...
            case Struct: {
                ...
                if (structuredType == CIPStructTypeCode.STRING.getValue()) {
                    ...
                    //  return as type STRUCT with structuredType to let user
    ...
    private PlcValue parsePlcValue(EipTag tag, ByteBuf data, CIPDataTypeCode type) {
        ...
        if (nb > 1) {
            ...
            for (int i = 0; i < nb; i++) {
                pezzoCod(data, type, list, STRING_LEN_OFFSET,  STRING_DATA_OFFSET, index);
            }
    ...
    public CompletableFuture<PlcWriteResponse> write(PlcWriteRequest writeRequest) {
        ...
        for (String tagName : request.getTagNames()) {
            pezzoCod2(tagName, request, items);
         }
         RequestTransactionManager.RequestTransaction transaction = tm.startRequest();
         if (items.size() == 1) {
             tm.startRequest();
             ...
             transaction.submit(() -> context.sendRequest(rrdata)
                 ...
                 .onTimeout(future::completeExceptionally)
                 .onError((p, e) -> future.completeExceptionally(e))
                 .check(CipRRData.class::isInstance).unwrap(CipRRData.class::cast)
                 .check(p -> p.getSessionHandle() == sessionHandle)
                 //.check(p -> p.getSenderContext() == senderContext)
...
private  void  pezzoCod2(String tagName,  DefaultPlcWriteRequest request, List<CipWriteRequest> items){
            ...
            byte[] data = encodeValue(value, tag.getType());
    ...
    private PlcResponse decodeWriteResponse(CipService p, PlcWriteRequest writeRequest) {
        ...
        if (p instanceof CipWriteResponse) {
            ...
            int total = read.getTotalBytes();
            pezzoCod1(resp, total, read, arr, nb);///////////////////////////////////////////////
    ...
    private void  pezzoCod1( MultipleServiceResponse resp, int total, ReadBufferByteBased read, List<CipService> arr, int nb){
        for (int i = 0; i < nb; i++) {
                int length = 0;
    ...
    private PlcResponseCode decodeResponseCode(int status) {
        ...
        else return PlcResponseCode.INTERNAL_ERROR;
    }
