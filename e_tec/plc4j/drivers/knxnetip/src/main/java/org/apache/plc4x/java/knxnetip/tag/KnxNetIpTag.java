...
public class KnxNetIpTag implements PlcTag {
    ...
    private static final String S1="mainGroup";
    ...
    public static KnxNetIpTag of(String tagString) {
        ...
        if(matcher.matches()) {
            return new KnxNetIpTag(1, matcher.group(S1), null, null);
        }
        matcher = KNX_GROUP_ADDRESS_2_LEVEL.matcher(tagString);
        if(matcher.matches()) {
            return new KnxNetIpTag(2, matcher.group(S1), null, matcher.group("subGroup"));
        }
        matcher = KNX_GROUP_ADDRESS_3_LEVEL.matcher(tagString);
        if(matcher.matches()) {
            return new KnxNetIpTag(3, matcher.group(S1), matcher.group("middleGroup"), matcher.group("subGroup"));
        }
