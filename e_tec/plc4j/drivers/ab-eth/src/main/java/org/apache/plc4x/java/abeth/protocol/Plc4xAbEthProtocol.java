...
public class Plc4xAbEthProtocol extends PlcMessageToMessageCodec<CIPEncapsulationPacket, PlcRequestContainer> {
    ...
    public PlcResponse decodeReadResponse(
        ...
        for (String tagName : plcReadRequest.getTagNames()) {
            ...
            nuovoCod1(plcReadResponse, responseCode, tagName, tag);
            ResponseItem<PlcValue> result = new ResponseItem<>(responseCode, plcValue);
            values.put(tagName, result);
        }
        return new DefaultPlcReadResponse(plcReadRequest, values);
    }
    public void nuovoCod1(CIPEncapsulationReadResponse plcReadResponse, PlcResponseCode responseCode, String tagName, AbEthTag tag){
        if (responseCode == PlcResponseCode.OK) {
            try {
                    nuovoCod2(plcReadResponse, tag, tagName);
                }
                catch (Exception e) {
    ...
    public void nuovoCod2(CIPEncapsulationReadResponse plcReadResponse, AbEthTag tag, String tagName){
        ...
        switch (tag.getFileType()) {
            ...
            case DWORD:
                if(plcReadResponse.getResponse() instanceof DF1CommandResponseMessageProtectedTypedLogicalRead) {
                    ...
                    List<Short> data = df1PTLR.getData();
                    if (((data.get(3)>> 7) & 1) == 0)  {
                        PlcValueHandler1.of((data.get(3) << 24) + (data.get(2) << 16) + (data.get(1) << 8) + data.get(0));  // positive number
                    } else {
                        PlcValueHandler1.of((((~data.get(3) & 0b01111111) << 24) + ((~(data.get(2)-1) & 0b11111111) << 16)+ ((~(data.get(1)-1) & 0b11111111) << 8) + (~(data.get(0)-1) & 0b11111111))  * -1);  // negative number
                    }
                }
                break;
            case SINGLEBIT:
                if(plcReadResponse.getResponse() instanceof DF1CommandResponseMessageProtectedTypedLogicalRead) {
                    ...
                    if (tag.getBitNumber() < 8) {
                        PlcValueHandler1.of((data.get(0) & (1 <<  tag.getBitNumber())) != 0);// read from first byte
                    } else {
                        PlcValueHandler1.of((data.get(1) & (1 << (tag.getBitNumber() - 8) )) != 0);
