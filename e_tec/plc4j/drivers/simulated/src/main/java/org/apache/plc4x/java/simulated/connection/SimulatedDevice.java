...
public class SimulatedDevice {
    ...
    public void set(SimulatedTag tag, PlcValue value) {
        ...
        switch (tag.getType()) {
            case STATE:
                ...
                return;
            case STDOUT:
                String format = "TEST PLC STDOUT : {}" + tag.getName() + value;
                LOGGER.info(format);
                return;
            case RANDOM:
                switch (tag.getPlcValueType()) {
                    ...
                    default:
                        try {
                            ...
                            final WriteBufferByteBased writeBuffer = new WriteBufferByteBased((int) Math.ceil((lengthInBits) / 8.0f));
    ...
    public void addCyclicSubscription(Consumer<PlcValue> consumer, PlcSubscriptionHandle handle, PlcSubscriptionTag subscriptionTag, Duration duration) {
        ...
        if(subscriptionTag instanceof DefaultPlcSubscriptionTag)
            throw new IllegalArgumentException("Excpetion message");
