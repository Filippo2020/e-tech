...
public abstract class ModbusProtocolLogic<T extends ModbusADU> extends Plc4xProtocolBase<T> {
    String frase = "Unexpected response type. ";
    ...
    protected ModbusProtocolLogic(DriverType driverType) {
    ...
    @Override
    public void decode(ConversationContext<T> context, T msg) throws IllegalArgumentException{
        try{
    ...
    protected PlcValue toPlcValue(ModbusPDU request, ModbusPDU response, ModbusDataType dataType) throws ParseException {
        ...
        String respClassName=response.getClass().getName();
        if (request instanceof ModbusPDUReadDiscreteInputsRequest) {
            if (!(response instanceof ModbusPDUReadDiscreteInputsResponse)) {
                throw new PlcRuntimeException(frase +
                    "Expected ModbusPDUReadDiscreteInputsResponse, but got " +respClassName);
            }
            ...
            if (!(response instanceof ModbusPDUReadFileRecordResponse)) {
                throw new PlcRuntimeException(frase +
                    "Expected ModbusPDUReadFileRecordResponse, but got " + respClassName);
            }
