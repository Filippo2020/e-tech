...
public class AdsProtocolLogic extends Plc4xProtocolBase<AmsTCPPacket> implements HasConfiguration<AdsConfiguration>, PlcSubscriber, PlcBrowser {
    ...
    private String frase = "Tags are null";

    private static final String S1="WSTRING(";
    private static final String S2="Unexpected result ";
    private static final String S3="symbolVersion";
    private static final String S4="STRING(";
    private static final String S5="Result is ";
    private static final String S6="onlineVersion";
    ...
    public void onConnect(ConversationContext<AmsTCPPacket> context) {
        ...
        CompletableFuture<Void> setupAmsRouteFuture = new CompletableFuture<>();
                                    ...
                                    readSymbolVersionNumberTx.submit(() -> context.sendRequest(new AmsTCPPacket(readSymbolVersionNumberRequest))
                                        ...
                                        .handle(readSymbolVersionNumberResponse -> {
                                            ...
                                            if (readSymbolVersionNumberResponse.getResult() != ReturnCode.OK) {
                                                // : Handle this
                                                future.completeExceptionally(new PlcException(S5 + readSymbolVersionNumberResponse.getResult()));
                                                return;
                                            }
                        ...
                        .handle(readDataTypeTableResponse -> {
                            ...
                            readSymbolTableTx.submit(() -> context.sendRequest(amsReadSymbolTableTCPPacket)
                                ...
                                .handle(readSymbolTableResponse -> {
                                    ...
                                    subscriptionTags.put(S6, new DefaultPlcSubscriptionTag(
                                    ...
                                    subscriptionTags.put(S3, new DefaultPlcSubscriptionTag(
                                        PlcSubscriptionType.CHANGE_OF_STATE,
                                        new DirectAdsTag(0xF008, 0x0000, "USINT", 1),
                                        Duration.ofMillis(1000)));
                                    LinkedHashMap<String, List<Consumer<PlcSubscriptionEvent>>> consumer = new LinkedHashMap<>();
                                    consumer.put(S6, Collections.singletonList(plcSubscriptionEvent -> {
                                        long oldVersion = onlineVersion;
                                        long newVersion = plcSubscriptionEvent.getPlcValue(S6).getLong();
                                        
                                        cod1(oldVersion, newVersion, invalidationLock, context);

                                    }));
                                    consumer.put(S3, Collections.singletonList(plcSubscriptionEvent -> {
                                        int oldVersion = symbolVersion;
                                        int newVersion = plcSubscriptionEvent.getPlcValue(S3).getInteger();
                                        
                                        cod4(oldVersion, newVersion, invalidationLock, context);
    ...
    public void cod1(long oldVersion, long newVersion, ReentrantLock invalidationLock, ConversationContext<AmsTCPPacket> context){
    ...
    public void cod2(AdsTableSizes adsTableSizes, ReadBuffer rb2){
    ...
    public void cod3(AdsTableSizes adsTableSizes, ReadBuffer rb){
    ...
    public void cod4(long oldVersion, long newVersion, ReentrantLock invalidationLock, ConversationContext<AmsTCPPacket> context){
        if(oldVersion != newVersion) {
    ...
    public CompletableFuture<PlcBrowseResponse> browseWithInterceptor(PlcBrowseRequest browseRequest, PlcBrowseRequestInterceptor interceptor) {
        ...
        for (String queryName : browseRequest.getQueryNames()) {
            ...
            for (AdsSymbolTableEntry symbol : symbolTable.values()) {
                pezzoMetodo(symbol, interceptor, resultsForQuery);
            }
            responseCodes.put(queryName, PlcResponseCode.OK);
            values.put(queryName, resultsForQuery);
             } catch (RuntimeException e) {
            // : handle exception
            LOGGER.debug("Gotta implement this ... Currently ignoring all queries.");
            }
    ...
    public void pezzoMetodo(AdsSymbolTableEntry symbol, PlcBrowseRequestInterceptor interceptor, List<PlcBrowseItem> resultsForQuery){
        // Get the datatype of this entry.
        AdsDataTypeTableEntry dataType = dataTypeTable.get(symbol.getDataTypeName());
        if (dataType == null) {
           String format = "couldn't find datatype: %s%n" + symbol.getDataTypeName();
           LOGGER.info(format);
            return;
        }
        ...
        if(plc4xPlcValueType == org.apache.plc4x.java.api.types.PlcValueType.LI_ST) {
    ...
    protected List<PlcBrowseItem> getBrowseItems(String basePath, long baseGroupId, long baseOffset, boolean parentWritable, AdsDataTypeTableEntry dataType) {
        ...
        for (AdsDataTypeTableChildEntry child : dataType.getChildren()) {
            ...
            if (childDataType == null) {
                String format = "couldn't find datatype: %s%n" + child.getDataTypeName();
                logger.info(format);
                continue;
            }
    ...
    public CompletableFuture<PlcReadResponse> read(PlcReadRequest readRequest) {
        ...
        if (directAdsTagsFuture.isDone()) {
            ...
            if (resolvedTags != null) {
                ...
                errorFuture.completeExceptionally(new PlcException(frase));
                return errorFuture;
            }
        } else {
    ...
    protected CompletableFuture<PlcReadResponse> executeRead(PlcReadRequest readRequest,
        ...
        if (resolvedTags.size() == 1 && resolvedTags.values().stream().findFirst().isPresent()) {
            // Do a normal (single item) ADS Read Request
            Optional<DirectAdsTag> optional = resolvedTags.values().stream().findFirst();
            if(optional.isPresent())
                return singleRead(readRequest, optional.get());
        } else {
            //Check if the version of the remote station is at least TwinCAT v2.11 Build >= 1550 otherwise split up into single item requests.
        ...
        transaction.submit(() -> context.sendRequest(amsTCPPacket)
            ...
            .handle(response -> {
                if (response.getResult() == ReturnCode.OK) {
                    ...
                    future.completeExceptionally(new PlcException(S5 + response.getResult()));
                }
            ...
            .handle(response -> {
                if (response.getResult() == ReturnCode.OK) {
                    ...
                    if (response.getResult() == ReturnCode.ADSERR_DEVICE_INVALIDSIZE) {
                        ...
                        future.completeExceptionally(new PlcException(S2 + response.getResult()));
                    }
                }
                // Finish the request-transaction.
                transaction.endRequest();
    ...
    protected PlcReadResponse convertToPlc4xReadResponse(PlcReadRequest readRequest, AmsPacket adsData) {
        ...
        if (readBuffer != null) {
            cod1(readRequest, responseCodes, readBuffer);
        }
        return null;
    }

    protected PlcReadResponse cod1(PlcReadRequest readRequest, Map<String, PlcResponseCode> responseCodes, ReadBuffer readBuffer){
    ...
    private ResponseItem<PlcValue> parseResponseItem(DirectAdsTag tag, ReadBuffer readBuffer) {
        try {
            ...
            if (tag.getNumberOfElements() == 1) {
                ...
                return new ResponseItem<>(PlcResponseCode.OK, PlcValueHandler1.of(resultItems));
            }
        } catch (Exception e) {
    ...
    protected CompletableFuture<PlcWriteResponse> executeWrite(PlcWriteRequest writeRequest,
        ...
        if (resolvedTags.size() == 1 && resolvedTags.values().stream().findFirst().isPresent()) {
            ...
            Optional<DirectAdsTag> optional = resolvedTags.values().stream().findFirst();
            if(optional.isPresent())
                return singleWrite(writeRequest, optional.get());
        } else {
            //Check if the version of the remote station is at least TwinCAT v2.11 Build >= 1550 otherwise split up into single item requests.
            // Do a ADS-Sum Read Request.
    ...
    private CompletableFuture<PlcSubscriptionResponse> executeSubscribe(PlcSubscriptionRequest subscribeRequest, Map<AdsTag, DirectAdsTag> resolvedTags) {
        ...
        List<AmsTCPPacket> amsTCPPackets = subscribeRequest.getTags().stream()
            .map(DefaultPlcSubscriptionTag.class::cast)
            .map(tag -> {
                AdsDataTypeTableEntry adsDataTypeTableEntry = dataTypeTable.get(resolvedTags.get(tag.getTag()).getPlcDataType());
                DirectAdsTag directAdsTag = getDirectAdsTagForSymbolicName(tag.getTag());
                //We should implement multi-dimensional arrays here ...
            ...
            context.sendRequest(packet)
                ...
                .handle(response -> {
                    if (response.getResult() == ReturnCode.OK) {
                        ...
                        AdsDataTypeTableEntry adsDataTypeTableEntry = dataTypeTable.get((resolvedTags.get(subscriptionTag.getTag())).getPlcDataType());
    ...
    public CompletableFuture<PlcUnsubscriptionResponse> unsubscribe(PlcUnsubscriptionRequest unsubscriptionRequest) {
        ...
        unsubscriptionRequest.getSubscriptionHandles().stream()
            .filter(AdsSubscriptionHandle.class::isInstance)
            .map(AdsSubscriptionHandle.class::cast)
            ...
            context.sendRequest(packet)
                ...
                .handle(response -> {
                    if (response.getResult() == ReturnCode.OK) {
                        ...
                        // this is more guesswork than knowing it could actually occur
    ...
    @Override
    protected void decode(ConversationContext<AmsTCPPacket> context, AmsTCPPacket msg) {
        try{
    ...
    protected CompletableFuture<Map<AdsTag, DirectAdsTag>> getDirectAddresses(List<PlcTag> tags) {
        ...
        if (!symbolicTagsNeedingResolution.isEmpty()) {
            metodo1(tags, symbolicTagsNeedingResolution, future);
        } else {
    ...
    public CompletableFuture<Map<AdsTag, DirectAdsTag>> metodo1(List<PlcTag> tags, List<SymbolicAdsTag> symbolicTagsNeedingResolution, CompletableFuture<Map<AdsTag, DirectAdsTag>> future){
    ...
    public void codN(List<SymbolicAdsTag> requiredResolutionTags){
