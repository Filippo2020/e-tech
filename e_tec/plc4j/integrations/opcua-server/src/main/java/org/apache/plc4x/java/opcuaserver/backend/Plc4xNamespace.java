...
public class Plc4xNamespace extends ManagedNamespaceWithLifecycle {
    ...
    private void addConfiguredNodes(UaFolderNode rootNode, DeviceConfiguration c) {
        ...
        for (int i = 0; i < tags.size(); i++) {
            String format = "Adding Tag %s - %s" + tags.get(i).getAlias() + tags.get(i).getAddress();
            logger.info(format);
            ...
            try {
                ...
                node = cod1(length, name, typeId, datatype);
                ...
                node.getFilterChain().addLast(
                    AttributeFilters.getValue(
                        ctx -> {
                            try {
                                return plc4xServer.getValue(ctx, tag, connectionString);
                            } catch (Exception e) {
                                String f = "An error occurre" + e;
                                logger.info(f);
                            }
                            return null;
                        }
    ...
    private UaVariableNode cod1(int length, String name, NodeId typeId, Class datatype){
        UaVariableNode node2;
        if (length > 1) {
            node2 = new UaVariableNode.UaVariableNodeBuilder(getNodeContext())
    ...
    public void onMonitoringModeChanged(List<MonitoredItem> monitoredItems) {
        try {
            throw new IllegalAccessException(" 4 - " + monitoredItems.toString());
        ...
        for (MonitoredItem item : monitoredItems) {
            try {
                throw new IllegalAccessException(" 4 - " + item.toString());
            } catch (IllegalAccessException e) {
                String format = "An error occurre!" + e;
                logger.info(format);
            }
        }
