...
public class OPCUAServer {
    ...
    private static  final String S1="set-passwords";
    private static  final String S2="interactive";
    private static  final String S3="127.0.0.1";
    ...
    protected String[] setPasswords() {
        ...
        logger.debug("Please enter password for certificate:- ");
        ret[0] = String.valueOf(cnsl.readPassword());

        logger.debug("Please enter a username for the OPC UA server admin account:- ");
        ret[1] = String.valueOf(cnsl.readLine());

        logger.debug("Please enter a password for the OPC UA server admin account:- ");
    ...
    private void setPasswordWrapper() {
        ...
        try {
            ...
            logger.error(String.format("Unable to save config file, please check folder permissions. %s", e));
            System.exit(1);
        }
    }
    ...
    private void readPasswordConfig() {
        ...
        try {
            ...
            logger.info(String.format("Error parsing password file %s", e));
        }
    }
    ...
    private void readCommandLineArgs(String[] args) {
        ...
        Option setPassword = new Option("s", S1, false, "Reset passwords");
        setPassword.setRequired(false);
        options.addOption(setPassword);

        Option interactive = new Option("i", S2, false, "Interactively get asked to setup the config file from the console");
        ...
        try {
            ...
            logger.info(String.format("Error parsing config file %s",e));
            System.exit(1);
        }
    }
    ...
    private static final String CERTIFICATEFILENAME = "plc4x-opcuaserver.pfx";

    public OPCUAServer(String[] args) throws IOException, KeyStoreException, NoSuchAlgorithmException, CertificateException, UnrecoverableKeyException{
        ...
        if (!securityTempDir.exists() && !securityTempDir.mkdirs()) {
                throw new IllegalStateException("Unable to create directory please confirm folder permissions on " + securityTempDir.toString());
        }
        ...
        UsernameIdentityValidator identityValidator = new UsernameIdentityValidator(
            ...
            authChallenge -> {
                ...
                if (!check) {
                    String format = "Invalid password for user:- %s" + authChallenge.getUsername();
