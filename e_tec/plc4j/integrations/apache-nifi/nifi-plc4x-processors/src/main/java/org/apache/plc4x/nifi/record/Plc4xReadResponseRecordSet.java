...
public class Plc4xReadResponseRecordSet implements RecordSet, Closeable {
    ...
    // review this AtomicReference?
	// this could be enhanced checking if record schema should be updated (via a cache boolean, checking property values is a nifi expression language, etc)
    ...
    public Plc4xReadResponseRecordSet(final PlcReadResponse readResponse){
        ...
        if (recordSchema == null) {
        	Schema avroSchema = Plc4xCommon.createSchema(responseDataStructure); //review this method as it is the 'mapping' from PlcValues to avro datatypes        	
        	recordSchema = new AtomicReference<>();
    ...
    public Record next() throws IOException {
        if (moreRows) {
             final Record recordz = createRecord(readResponse);
             setMoreRows(false);
             return recordz;
        } else {
             return null;
    ...
    protected Record createRecord(final PlcReadResponse readResponse){
        ...
        for (final RecordField tag : getSchema().getFields()) {
            ...
            logger.debug("Adding %s tag value to record.");
    ...
    private Object normalizeValue(final PlcValue value) {
        ...
        if (r != null) {
            String format = "Value data type: %s" + r.getClass();
            try {
