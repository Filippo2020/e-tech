...
public class RecordPlc4xWriter implements Plc4xWriter {
	...
	@Override
	public long writePlcReadResponse(PlcReadResponse response, OutputStream outputStream, ComponentLog logger, Plc4xReadResponseRowCallback callback) throws IOException {
	...
	@Override
	public long writePlcReadResponse(PlcReadResponse response, OutputStream outputStream, ComponentLog logger, Plc4xReadResponseRowCallback callback, FlowFile originalFlowFile) throws IOException {
		if (fullRecordSet == null) {	
            try{
            fullRecordSet = new Plc4xReadResponseRecordSetWithCallback(response, callback);
            writeSchema = recordSetWriterFactory.getSchema(originalAttributes, fullRecordSet.getSchema());
	...
	private static class Plc4xReadResponseRecordSetWithCallback extends Plc4xReadResponseRecordSet {
        ...
        public Record next() throws IOException {
                if (hasMoreRows()) {
                    ...
                    final Record recordz = createRecord(response);
                    setMoreRows(false);
                    if (callback != null) {
