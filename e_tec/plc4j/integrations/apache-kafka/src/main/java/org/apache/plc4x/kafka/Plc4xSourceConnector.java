...
private StringBuilder pezzoCod1(StringBuilder query, Source source ){
    for (JobReference jobReference : source.getJobReferences()) {
        Job job = sourceConfig.getJob(jobReference.getName());
        if(job == null){
            try {
                throw new IllegalAccessException(String.format("Couldn't find referenced job '%s'", jobReference.getName()));
