...
public class ExpressionStringListener extends ExpressionBaseListener {
    ...
    private Deque<List<Term>> parserContexts;

    private Deque<CompletableFuture<TypeReference>> futureStack;
    ...
    public void exitExpressionString(ExpressionParser.ExpressionStringContext ctx) {
        ...
        if (roots.isEmpty()) {
            throw new IllegalArgumentException("Empty Expression not supported.");
        }
        if (roots.size() != 1) {
            throw new IllegalArgumentException("Expression can only contain one root term.");
        }
        root = roots.get(0);
    }
    ...
    public void exitIdentifierSegment(ExpressionParser.IdentifierSegmentContext ctx) {
        ...
        // Based on the current context type-definition, get the type of the property with name ctx.name.getText()

        Integer index = null;
        if (indexContext != null) {
            // Add a check, that the field providing the property is an "array" or "manualArray" field.
            index = indexContext.getFirst().getNumber().intValue();
        }
        VariableLiteral rest = null;
        if (restContext != null) {
            //Add a check, that the field providing the property references a complex type (or uses one of the built-ins)
    ...
    private UnaryTerm getUnaryTerm(String op, List<Term> terms) {
        if (terms.size() != 1) {
            throw new IllegalArgumentException(op + " should be a unary operation");
        }
        Term a = terms.get(0);
    ...
    private BinaryTerm getBinaryTerm(String op, List<Term> terms) {
        if (terms.size() != 2) {
            throw new IllegalArgumentException(op + " should be a binary operation");
        }
        Term a = terms.get(0);
        Term b = terms.get(1);
    ...
    private TernaryTerm getTernaryTerm(String op, List<Term> terms) {
        if (terms.size() != 3) {
            throw new IllegalArgumentException(op + " should be a ternary operation");
        }
        Term a = terms.get(0);
        Term b = terms.get(1);
        Term c = terms.get(2);
