...
public class MessageFormatListener extends MSpecBaseListener implements LazyTypeDefinitionConsumer {
    ...
    String error1 = "Error setting type for {}";
    private static final String S1="propertySizeInBits";
    protected Map<String, List<Consumer<TypeDefinition>>> typeDefinitionConsumers;

    private final Deque<Map<String, Term>> batchSetAttributes = new ArrayDeque<>();
    ...
    public void enterAbstractField(MSpecParser.AbstractFieldContext ctx) {
        ...
        getTypeReference(ctx.type).whenComplete((typeReference, throwable) -> {
            if (throwable != null) {
                //proper error collection in type context error bucket
                LOGGER.debug(error1, field, throwable);
                return;
            }
    ...
    public void exitCaseStatement(MSpecParser.CaseStatementContext ctx) {
        String namePrefix = "";
        // maybe name this prefix and suffix wildcard
    ...
    public void enterEnumValueDefinition(MSpecParser.EnumValueDefinitionContext ctx) {
        ...
        if (ctx.constantValueExpressions != null) {
            ...
            if (numConstantValues != numExpressionValues) {
                throw new IllegalArgumentException("Number of constant value expressions doesn't match the number of " +
    ...
    private Term getExpressionTerm(MSpecParser.ExpressionContext expressionContext) {
        ...
        try {
            ...
            throw new IllegalArgumentException(String.format("Error parsing expression: '%s' at line %d column %d",
    ...
    private VariableLiteral getVariableLiteral(MSpecParser.VariableLiteralContext variableLiteralContext) {
        ...
        try {
            ...
            throw new IllegalArgumentException(String.format("Error parsing variable literal: '%s' at line %d column %d",
        ...
        if (ctx.simpleTypeReference != null) {
            ...
            setOrScheduleTypeDefinitionConsumer(typeRefName, typeDefinition -> {
                if (typeDefinition.isDataIoTypeDefinition()) {
                    ...
                    throw new IllegalArgumentException("Support for " + typeDefinition.getClass() + " not implemented yet");
                }
            });
    ...
    private SimpleTypeReference getSimpleTypeReference(MSpecParser.DataTypeContext ctx) {
        ...
        switch (simpleBaseType) {
            ...
            case VINT: {
                ...
                if (attributes.containsKey(S1)) {
                    final Term propertySizeInBitsTerm = attributes.get(S1);
                    if (!(propertySizeInBitsTerm instanceof NumericLiteral)) {
                        throw new IllegalArgumentException("'propertySizeInBits' attribute is required to be a numeric literal");
                    }
        ...
        return params.stream()
            .map(argumentContext -> {
                ...
                getTypeReference(argumentContext.type).whenComplete((typeReference, throwable) -> {
                    if (throwable != null) {
                        //proper error collection in type context error bucket
                        LOGGER.debug(error1, argument, throwable);
                        return;
                    }
    ...
    private Term parseExpression(String expressionString) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Error parsing expression: '" + expressionString + "'", e);
        }
    }
    ...
    public void dispatchType(String typeName, TypeDefinition type) {
        ...
        if (types.containsKey(typeName)) {
            ...
            // we need to implement replace logic... means we need to replace all old references with the new one in that case otherwise we just get an exception
        }
