...
public abstract class BaseFreemarkerLanguageTemplateHelper implements FreemarkerLanguageTemplateHelper {
    ...
    protected EnumTypeDefinition getEnumTypeDefinition(TypeReference typeReference) {
        ...
        //same here. It is named complex type reference but it references a enum...
    ...
    public boolean hasFieldsWithNames(List<Field> fields, String... names) {
        ...
        //document why true is returned here.
        return true;
    }
    // check or describe why a instanceOf EnumField is not sufficient here
    ...
    public Map<String, TypeReference> getDiscriminatorTypes() {
        ...
        for (VariableLiteral variableLiteral : switchField.getDiscriminatorExpressions()) {
            ...
            if (discriminatorType.isEmpty()) {
                throw new IllegalArgumentException("no type for " + discriminatorName);
            }
    ...
    public boolean discriminatorValueNeedsStringEqualityCheck(Term term, TypeDefinition typeDefinition) {
        ...
        if (typeDefinition instanceof ComplexTypeDefinition) {
            ...
            for (Field field : complexTypeDefinition.getFields()) {
                if ((field instanceof NamedField)&&(ifCoda(field, variableLiteral))) {
                            TypedField typedField = (TypedField) field;
    ...
    public static boolean ifCoda(Field field, VariableLiteral variableLiteral){
      return (((NamedField) field).getName().equals(variableLiteral.getName()))&&(field instanceof TypedField);
    }
    ...
    //replace that with term.isWildcard() (once the referenced wildcard term from build utils is used)
    public void isWildcard(Term term) {
    }
    /**
